...
class Visualizer:
    def __init__(self, size) -> None:
        ...
        self.explosion = pygame.transform.smoothscale(pygame.image.load('explosion.png'), (self.player_width, self.player_height))
    ...
    def update_scene(self, grid, player_num_grid, num_agents, heads):
        ...
        for head in heads:
            hX, hY = head
            if(hX == -1 and hY == -1):
                self.screen.blit(self.explosion, (self.old_head[i][0], self.old_head[i][1]))
                continue
            ...
            self.screen.blit(self.player_imgs[i][0], (x,y))
            self.old_head[i] = (x,y)
            i += 1
        #for video
